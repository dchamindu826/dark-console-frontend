...
import CommunityChat from './components/CommunityChat'; // Make sure path is correct
...
// ... (LiveStream Component Code - Keep as is) ...
const LiveStream = () => {
    // ... (Code as before) ...
        ...
        <section className="py-20 px-4 bg-black relative border-y border-zinc-900">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="aspect-video w-full rounded-2xl overflow-hidden border border-zinc-800 shadow-[0_0_50px_rgba(34,197,94,0.1)]">
                    <iframe className="w-full h-full" src={`https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1`} title="Live Stream" frameBorder="0" allowFullScreen></iframe>
                </div>
            </div>
        </section>
    );
...
// ... (FeaturedPackages Component Code - Keep as is) ...
        ...
        <section className="py-24 px-4 bg-zinc-900/30">
            ...
            <div className="max-w-7xl mx-auto">
                 <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-5xl font-black uppercase text-white">Elite <span className="text-[var(--gta-green)]">Packages</span></h2>
                        ...
                        <div key={service._id} className="group relative bg-black border border-zinc-800 rounded-2xl overflow-hidden hover:border-[var(--gta-green)] transition-all duration-300 hover:-translate-y-2">
                             <div className="aspect-[2/3] w-full relative overflow-hidden">
                                ...
                                <div className="absolute bottom-4 left-4">
                                    ...
                                    <p className="text-2xl font-black text-[var(--gta-green)] font-sans tracking-tight drop-shadow-md">{formatLKR(service.price)}</p>
                                </div>
                            </div>
                            <div className="p-5 relative">
                                <h3 className="text-lg font-black text-white uppercase leading-tight mb-2 group-hover:text-[var(--gta-green)] transition-colors line-clamp-1">{service.title}</h3>
                                ...
                                <button onClick={() => navigate('/services')} className="w-full py-3 bg-zinc-800 text-white font-bold uppercase text-xs rounded-lg hover:bg-[var(--gta-green)] hover:text-black transition-colors flex items-center justify-center gap-2">View Details <ArrowRight size={14}/></button>
                            </div>
                        </div>
                    ))}
                </div>
                <div className="text-center mt-12">
                    <button onClick={() => navigate('/services')} className="text-white border-b border-[var(--gta-green)] pb-1 hover:text-[var(--gta-green)] transition-colors uppercase text-sm font-bold tracking-widest">View All Store Items</button>
                </div>
            </div>
        </section>
    );
...
const LandingPage = () => {
  return (
    <div className="bg-[#09090b] min-h-screen text-white font-sans selection:bg-green-500/30">
      ...
      {/* 🔥 2. PLACE COMMUNITY CHAT HERE (Above Leaderboard) */}
      <CommunityChat />
